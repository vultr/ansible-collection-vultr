...
DOCUMENTATION = """
---
module: cdn_zone_push
short_description: Manages CDN push zones on Vultr
description:
  - Create, update and remove CDN push zones.
version_added: "1.15.0"
author: "Rene Moser (@resmo)"
options:
  label:
    description:
      - The label of the CDN push zone.
    required: true
    aliases: [ name ]
    type: str
  vanity_domain:
...
EXAMPLES = """
- name: Ensure a CDN push zone is present
  vultr.cloud.cdn_zone_push:
    label: my-push-zone
    gzip: true

- name: Ensure a CDN push zone is absent
  vultr.cloud.cdn_zone_push:
    label: my-push-zone
    state: absent
"""
...
vultr_cdn_zone_push:
  ...
  contains:
    id:
      description: ID of the CDN push zone.
      returned: success
      type: str
    label:
      description: Label of the CDN push zone.
      returned: success
      type: str
    status:
      description: Current CDN push zone status.
      returned: success
      type: str
    cdn_url:
